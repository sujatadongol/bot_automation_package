...
import {
  ...
  GetFinalResolvedValueList,
} from '../../containers/CreateIntentPage/helperFile';
import { ExtractWordWith$ } from '../../containers/CreateIntentPage/helper';
...
const TrainingPhraseIntentParam = ({
  ...
  const removeDuplicateInParam =
    paramId.length > 0 ? [...new Set(paramId)] : undefined;
  const removeDuplicateInResolved =
    resolvedValueId.length > 0 ? [...new Set(resolvedValueId)] : undefined;

  const intentParamWithParamName = [];
  const intentParamWithResolvedValue = [];

  const idExists = (arr, text) => {
    if (arr) {
      return (
        arr &&
  ...
  return (
    <CreateIntentComponentStyle>
      <div className="intent-row-wrapper">
        ...
        {GetFinalResolvedValueList(intentParamWithResolvedValue) &&
          GetFinalResolvedValueList(intentParamWithResolvedValue).length > 0 &&
          GetFinalResolvedValueList(intentParamWithResolvedValue).map(
            singleObj => {
              if (
                singleObj &&
                idExists(resolvedValueList, phraseText) &&
                phraseText.includes(singleObj.intentId)
              ) {
                return (
                  <Row gutter={20}>
                    <Col className="intent-list-data" span={1} />
                    <Col className="intent-list-data" span={8}>
                      {singleObj && singleObj.paramName}
                    </Col>
                    <Col className="intent-list-data" span={7}>
                      <span
                        style={{
                      ...
                      >
                        {' '}
                        @{singleObj && singleObj.entityName}
                      </span>
                    </Col>
                    <Col className="intent-list-data" span={7}>
                      {singleObj && singleObj.resolvedValue}
                    </Col>
